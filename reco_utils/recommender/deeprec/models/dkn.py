...
from examples.start_file.aggregators import *
__all__ = ["DKN"]
...
class DKN(BaseModel):
        ...
        with self.graph.as_default():
            with tf.name_scope("embedding"):
                ...
                if hparams.use_context:
                    ...
                    W = tf.Variable(
                        tf.random.uniform([hparams.dim, hparams.dim], -1, 1)
                    )
                    b = tf.Variable(tf.zeros([hparams.dim]))
    ...
    def _build_graph(self):
        ...
        with tf.compat.v1.variable_scope("DKN") as scope:
            logit = self._build_kgcn()
            return logit
    def _build_kgcn(self):
        hparams = self.hparams
    ...
    def build_em(self,candidate_word_batch,
        ...
        with tf.compat.v1.variable_scope(
            ...
            with tf.compat.v1.variable_scope(
                ...
                news_field_embed = self._new_em(
                    candidate_word_batch, candidate_entity_batch, hparams, isclick=False
                )
                candi_field_embed = self._candi_new_em(
                    click_word_batch, click_entity_batch, hparams, isclick=False
                )
            pred_one = tf.keras.layers.Dot(axes=-1)([news_field_embed, candi_field_embed])
            pred_one = tf.keras.layers.Activation(activation="sigmoid")(pred_one)
        return pred_one

    def _new_em(self,candidate_word_batch, candidate_entity_batch, hparams, isclick=False):

        dim = hparams.dim
        embedded_chars = tf.nn.embedding_lookup(self.embedding, candidate_word_batch)
        # embedded_entity = tf.nn.embedding_lookup(self.entity_embedding,candidate_entity_batch)

        candi_entities, candi_relations = self.get_neighbors(candidate_entity_batch, hparams)
        candi_item_embeddings, candi_aggregators = self.aggregate(candi_entities, candi_relations, hparams)

        embedded_chars = tf.reduce_mean(embedded_chars,axis=1)
        concat = tf.concat(
            [embedded_chars, candi_item_embeddings], axis=-1
        )
        return concat
    def _candi_new_em(self,click_word_batch, click_entity_batch, hparams, isclick=False):

        dim = hparams.dim
        embedded_chars = tf.nn.embedding_lookup(self.embedding, click_word_batch)
        # embedded_entity = tf.nn.embedding_lookup(self.entity_embedding,candidate_entity_batch)

        click_entities, click_relations = self.get_neighbors(click_entity_batch, hparams)
        click_item_embeddings, candi_aggregators = self.aggregate_click(click_entities, click_relations, hparams)

        embedded_chars = tf.reduce_mean(tf.reshape(embedded_chars,[100,-1,100]),axis=1)
        concat = tf.concat(
            [embedded_chars, click_item_embeddings], axis=-1
        )
        return concat
    def aggregate_click(self, entities, relations,hparams):
        aggregators = []  # store all aggregators
        entity_vectors = [tf.nn.embedding_lookup(self.entity_embedding, i) for i in entities]#[100,1,10,100],[100,40,100],[100,160,100]
        relation_vectors = [tf.nn.embedding_lookup(self.context_embedding, i) for i in relations]
        hparams.n_iter = 1
        hparams.n_neighbor = 4
        for i in range(hparams.n_iter):
            if i == hparams.n_iter - 1:
                aggregator = self.aggregator_class(hparams.batch_size, hparams.dim, act=tf.nn.tanh)
            else:
                aggregator = self.aggregator_class(hparams.batch_size, hparams.dim)
            aggregators.append(aggregator)

            entity_vectors_next_iter = []
            for hop in range(hparams.n_iter - i):
                shape = [hparams.batch_size, -1, hparams.n_neighbor, hparams.dim]
        ...
        res = tf.reshape(tf.reduce_sum(entity_vectors[0],axis=1), [hparams.batch_size, hparams.dim])

        return res, aggregators
    def get_neighbors(self, seeds,hparams):
        seeds = tf.expand_dims(seeds, axis=1)#[100,1,10]
        entities = [seeds]
        relations = []
        for i in range(2):             #[100,1,10],[100,40],[100,160]
            neighbor_entities = tf.reshape(tf.gather(hparams.adj_entity, entities[i]), [hparams.batch_size, -1])
            neighbor_relations = tf.reshape(tf.gather(hparams.adj_relation, entities[i]), [hparams.batch_size, -1])
            entities.append(neighbor_entities)
            relations.append(neighbor_relations)
        return entities, relations
    def aggregate(self, entities, relations,hparams):
        aggregators = []  # store all aggregators
        entity_vectors = [tf.nn.embedding_lookup(self.entity_embedding, i) for i in entities]#[100,1,10,100],[100,40,100],[100,160,100]
        relation_vectors = [tf.nn.embedding_lookup(self.context_embedding, i) for i in relations]
        hparams.n_iter = 1
        ...
        with tf.compat.v1.variable_scope(
            ...
            with tf.compat.v1.variable_scope(
                ...
                click_field_embed = self._kims_cnn(
                    clicked_words, clicked_entities, hparams,isclick=True
                )
    ...
    def _kims_cnn(self, word, entity, hparams,isclick):
        ...
        if hparams.use_entity and hparams.use_context:
            ...
            # )
            concat = tf.concat(
                [embedded_chars, entity_embedded_chars], axis=-1
            )
