...
class MINDAllIterator(BaseIterator):
    ...
    def init_news(self, news_file):
            ...
            for line in rd:
                ...
                for entity in json.loads(title_entity):
                    if entity['WikidataId'] in self.entity2index:
                        part_entity.append(entity['WikidataId'])
                for entity in json.loads(ab_entity):
                    if entity['WikidataId'] in self.entity2index:
                        part_entity.append(entity['WikidataId'])
                news_entity.append(part_entity)
        ...
        self.news_entity_index = np.zeros((len(news_entity),self.entity_size),dtype="int32")
        ...
        for news_index in range(len(news_title)):
            ...
            for entity_index in range(min(self.entity_size,len(entity))):
                if entity[entity_index] in self.entity2index:
                    self.news_entity_index[news_index,entity_index] = self.entity2index[
                        entity[entity_index]
                    ]
    ...
    def parser_one_line(self, line):
        ...
        if self.npratio > 0:
            ...
            for p in poss:
                ...
                candidate_entity_index = self.news_entity_index[[p] + n]
                ...
                click_entity_index = self.news_entity_index[self.histories[line]]
            ...
            for news, label in zip(impr, impr_label):
                ...
                candidate_entity_index = self.news_entity_index[news]
                click_title_index = self.news_title_index[self.histories[line]]
                click_ab_index = self.news_ab_index[self.histories[line]]
                click_vert_index = self.news_vert_index[self.histories[line]]
                click_subvert_index = self.news_subvert_index[self.histories[line]]
    ...
    def load_data_from_file(self, news_file, behavior_file):
        ...
        candidate_entity_indexes = []
        click_title_indexes = []
        click_ab_indexes = []
        click_vert_indexes = []
        click_subvert_indexes = []
        click_entity_indexes = []
        cnt = 0
        ...
        for index in indexes:
            for (
                ...
                candidate_entity_indexes.append(candidate_entity_index)
                ...
                click_entity_indexes.append(click_entity_index)
    ...
    def _convert_data(
        ...
        candidate_entity_index_batch = np.asarray(candidate_entity_indexes,dtype=np.int64)
        ...
        click_entity_index_batch = np.asarray(click_entity_indexes,dtype=np.int64)
        return {
            ...
            "clicked_entity_batch":click_entity_index_batch,
            ...
            "candidate_entity_batch": candidate_entity_index_batch,
            "labels": labels,
        }
    ...
    def load_user_from_file(self, news_file, behavior_file):
        ...
        for index in range(len(self.impr_indexes)):
            ...
            click_entity_indexes.append(self.news_entity_index[self.histories[index]])
    ...
    def load_news_from_file(self, news_file):
        ...
        for index in range(len(self.news_title_index)):
            ...
            candidate_entity_indexes.append(self.news_entity_index[index])
    ...
    def _convert_news_data(
        ...
        candidate_entity_index_batch = np.asarray(candidate_entity_indexes,dtype=np.int32)
