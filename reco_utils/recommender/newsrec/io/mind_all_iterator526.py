...
class MINDAllIterator(BaseIterator):
    ...
    def __init__(
        ...
        self.entity2index = self.load_dict((hparams.entityDict_file))
        self.entity_neighbors = self.load_dict(hparams.entity_neighbors_file)
        print("load done")
    ...
    def init_news(self, news_file):
        ...
        title_news_entity = [""]
        title_news_neighbor = [""]
        abstract_news_entity = [""]
        abstract_news_neighbor = [""]
        with tf.io.gfile.GFile(news_file, "r") as rd:
            for line in rd:
                ...
                title_part_entity = []
                title_news_neighbor_part = []
                abstract_part_entity = []
                abstract_news_neighbor_part = []
                for entity in json.loads(title_entity):
                    if entity['WikidataId'] in self.entity2index:
                        title_part_entity.append(entity['WikidataId'])
                        for item in self.entity_neighbors[entity['WikidataId']]:
                            if item in self.entity2index:
                                title_news_neighbor_part.append(item)
                            else:
                                print('tiitem',item)
                for entity in json.loads(ab_entity):
                    if entity['WikidataId'] in self.entity2index:
                        abstract_part_entity.append(entity['WikidataId'])
                        for item in self.entity_neighbors[entity['WikidataId']]:
                            if item in self.entity2index:
                                abstract_news_neighbor_part.append(item)
                            else:
                                print('abitem',item)
                # news_entity.append(list(set(part_entity)))
                title_news_entity.append(title_part_entity)
                title_news_neighbor.append(list(set(title_news_neighbor_part)))
                abstract_news_entity.append(abstract_part_entity)
                abstract_news_neighbor.append(list(set(abstract_news_neighbor_part)))
                if nid in self.nid2index:
                    continue
                #加个判断，如果WikidataId在数据集里，就加入，否则不加入
        ...
        self.title_news_entity_index = np.zeros((len(title_news_entity),self.entity_size),dtype="int32")
        self.title_news_neighbor_index = np.zeros((len(title_news_neighbor), self.neighbor_size), dtype="int32")
        self.abstract_news_entity_index = np.zeros((len(abstract_news_entity), self.entity_size), dtype="int32")
        self.abstract_news_neighbor_index = np.zeros((len(abstract_news_neighbor), self.neighbor_size), dtype="int32")
        ...
        for news_index in range(len(news_title)):
            ...
            title_entity = title_news_entity[news_index]
            title_neighbor = title_news_neighbor[news_index]
            abstract_entity = abstract_news_entity[news_index]
            abstract_neighbor = abstract_news_neighbor[news_index]
            ...
            if subvert in self.subvert_dict:
                ...
                #entity在这一步，不应该只包含自身实体，还需要有邻居实体，也就是在这一步，通过查找entity中的实体的邻居，添加至列表，
                #然后，执行下面这个函数。。所以，我首先需要构造一个文件，该文件包含，每个实体的一阶邻居。
            for entity_index in range(min(self.entity_size,len(title_entity))):
                if title_entity[entity_index] in self.entity2index:
                    self.title_news_entity_index[news_index,entity_index] = self.entity2index[
                        title_entity[entity_index]
                    ]
            for entity_index in range(min(self.neighbor_size,len(title_neighbor))):
                if title_neighbor[entity_index] in self.entity2index:
                    self.title_news_neighbor_index[news_index,entity_index] = self.entity2index[
                        title_neighbor[entity_index]
                    ]
            for entity_index in range(min(self.entity_size,len(abstract_entity))):
                if abstract_entity[entity_index] in self.entity2index:
                    self.title_news_entity_index[news_index,entity_index] = self.entity2index[
                        abstract_entity[entity_index]
                    ]
            for entity_index in range(min(self.neighbor_size,len(abstract_neighbor))):
                if abstract_neighbor[entity_index] in self.entity2index:
                    self.abstract_news_neighbor_index[news_index,entity_index] = self.entity2index[
                        abstract_neighbor[entity_index]
                    ]
        print('loging done')
    ...
    def init_behaviors(self, behaviors_file):
        ...
        with tf.io.gfile.GFile(behaviors_file, "r") as rd:
            ...
            for line in rd:
                ...
                impr_index += 1
        print("hello,this is init_behavior")
    def parser_one_line(self, line):
        ...
        if self.npratio > 0:
            ...
            for p in poss:
                ...
                candidate_title_entity_index = self.title_news_entity_index[[p] + n]
                candidate_title_neighbor_index = self.title_news_neighbor_index[[p] + n]
                candidate_abstract_entity_index = self.abstract_news_entity_index[[p] + n]
                candidate_abstract_neighbor_index = self.abstract_news_neighbor_index[[p] + n]
                ...
                click_title_entity_index = self.title_news_entity_index[self.histories[line]]
                click_title_neighbor_index = self.title_news_neighbor_index[self.histories[line]]
                click_abstract_entity_index = self.abstract_news_entity_index[self.histories[line]]
                click_abstract_neighbor_index = self.abstract_news_neighbor_index[self.histories[line]]
                ...
                yield (
                    ...
                    candidate_title_neighbor_index,
                    candidate_abstract_entity_index,
                    candidate_abstract_neighbor_index,
                    click_title_index,
                    click_ab_index,
                    click_vert_index,
                    click_subvert_index,
                    click_title_entity_index,
                    click_title_neighbor_index,
                    click_abstract_entity_index,
                    click_abstract_neighbor_index,
                )
            ...
            for news, label in zip(impr, impr_label):
                ...
                candidate_title_entity_index = self.title_news_entity_index[news]
                candidate_title_neighbor_index = self.title_news_neighbor_index[news]
                candidate_abstract_entity_index = self.abstract_news_entity_index[news]
                candidate_abstract_neighbor_index = self.abstract_news_neighbor_index[news]
                click_title_index = self.news_title_index[self.histories[line]]
                click_ab_index = self.news_ab_index[self.histories[line]]
                click_vert_index = self.news_vert_index[self.histories[line]]
                click_subvert_index = self.news_subvert_index[self.histories[line]]
    ...
    def load_data_from_file(self, news_file, behavior_file):
        ...
        candidate_title_neighbor_indexes = []
        candidate_abstract_entity_indexes = []
        candidate_abstract_neighbor_indexes = []
        click_title_indexes = []
        click_ab_indexes = []
        click_vert_indexes = []
        click_subvert_indexes = []
        click_title_entity_indexes = []
        click_title_neighbor_indexes = []
        click_abstract_entity_indexes = []
        click_abstract_neighbor_indexes = []
        cnt = 0
        ...
        for index in indexes:
            for (
                ...
                candidate_title_entity_indexes.append(candidate_title_entity_index)
                candidate_title_neighbor_indexes.append(candidate_title_neighbor_index)
                candidate_abstract_entity_indexes.append(candidate_title_entity_index)
                candidate_abstract_neighbor_indexes.append(candidate_title_neighbor_index)
                ...
                click_title_entity_indexes.append(click_title_entity_index)
                click_title_neighbor_indexes.append(click_title_neighbor_index)
                click_abstract_entity_indexes.append(click_abstract_entity_index)
                click_abstract_neighbor_indexes.append(click_abstract_neighbor_index)
    ...
    def _convert_data(
        ...
        Args:
            ...
            candidate_entity_indexes(list): the candidate news entitys's indices
            candidate_neighbor_indexes(list): the candidate news neighbors's indices
            ...
            click_entity_indexes (list):indices for user's clicked news entitys.
            click_neighbor_indexes (list):indices for user's clicked news neighbors
        ...
        candidate_title_entity_index_batch = np.asarray(candidate_title_entity_indexes,dtype=np.int64)
        candidate_title_neighbor_index_batch = np.asarray(candidate_title_neighbor_indexes,dtype=np.int64)
        candidate_abstract_entity_index_batch = np.asarray(candidate_abstract_entity_indexes, dtype=np.int64)
        candidate_abstract_neighbor_index_batch = np.asarray(candidate_abstract_neighbor_indexes, dtype=np.int64)
        ...
        click_title_neighbor_index_batch = np.asarray(click_title_neighbor_indexes,dtype=np.int64)
        click_abstract_entity_index_batch = np.asarray(click_abstract_entity_indexes, dtype=np.int64)
        click_abstract_neighbor_index_batch = np.asarray(click_abstract_neighbor_indexes, dtype=np.int64)
        return {
            ...
            "clicked_title_neighbor_batch":click_title_neighbor_index_batch,
            "clicked_abstract_entity_batch": click_abstract_entity_index_batch,
            "clicked_abstract_neighbor_batch": click_abstract_neighbor_index_batch,
            ...
            "candidate_title_neighbor_batch": candidate_title_neighbor_index_batch,
            "candidate_abstract_entity_batch": candidate_abstract_entity_index_batch,
            "candidate_abstract_neighbor_batch": candidate_abstract_neighbor_index_batch,
            "labels": labels,
        }
    ...
    def load_user_from_file(self, news_file, behavior_file):
        ...
        for index in range(len(self.impr_indexes)):
            ...
            click_title_entity_indexes.append(self.title_news_entity_index[self.histories[index]])
            click_title_neighbor_indexes.append(self.title_news_neighbor_index[self.histories[index]])
            click_abstract_entity_indexes.append(self.abstract_news_entity_index[self.histories[index]])
            click_abstract_neighbor_indexes.append(self.abstract_news_neighbor_index[self.histories[index]])
    ...
    def _convert_user_data(
        ...
        Args:
            ...
            click_entity_indexes (list): words indices for user's clicked news entitys
    ...
    def load_news_from_file(self, news_file):
        ...
        for index in range(len(self.news_title_index)):
            ...
            candidate_title_entity_indexes.append(self.title_news_entity_index[index])
            candidate_title_neighbor_indexes.append(self.title_news_neighbor_index[index])
            candidate_abstract_entity_indexes.append(self.abstract_news_entity_index[index])
            candidate_abstract_neighbor_indexes.append(self.abstract_news_neighbor_index[index])
    ...
    def _convert_news_data(
        ...
        Args:
            ...
            candidate_entity_indexes (list): the candidate news entitys' words indices
        ...
        candidate_title_entity_index_batch = np.asarray(candidate_title_entity_indexes,dtype=np.int32)
        candidate_title_neighbor_index_batch = np.asarray(candidate_title_neighbor_indexes,dtype=np.int32)
        candidate_abstract_entity_index_batch = np.asarray(candidate_abstract_entity_indexes, dtype=np.int32)
        candidate_abstract_neighbor_index_batch = np.asarray(candidate_abstract_neighbor_indexes, dtype=np.int32)
