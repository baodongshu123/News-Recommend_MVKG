...
class MINDIterator(BaseIterator):
    ...
    def init_news(self, news_file):
        ...
        with tf.io.gfile.GFile(news_file, "r") as rd:
            for line in rd:
                ...
                #对新闻title进行切分,得到的是一句话被切分成很多个单词
                title = word_tokenize(title)
                news_title.append(title)
        #得到的结果是有uid还有title的编码
        self.news_title_index = np.zeros(
            (len(news_title), self.title_size), dtype="int32"
        )
        #self.news_title_index是新闻的 条数*size  18724*30
        #下面的函数是判断title的长度是不是小于30
    ...
    def parser_one_line(self, line):
        ...
        if self.npratio > 0:
            ...
            for p in poss:
                ...
                #news_title_index代表一个新闻所包含的word
                #candidate_title_index 代表选取的新闻+label为0的四个新闻，共五个，
                candidate_title_index = self.news_title_index[[p] + n]
                #click_title_index的值是[click,impr],中的click的50个单词
    ...
    def load_news_from_file(self, news_file):
        ...
        if not hasattr(self, "news_title_index"):
            self.init_news(news_file)
        #所以init_news得到的结果是news_title_Index就是每个标题通过tokensize切分以后，在word2index中查找到位置，得到的数组
        news_indexes = []
        candidate_title_indexes = []
        cnt = 0
        #news_title_index对每个新闻title编码
