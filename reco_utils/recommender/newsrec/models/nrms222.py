...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder):
        ...
        """
        hparams = self.hparams
        his_input_title_body_verts = tf.keras.Input(
            shape=(hparams.his_size, hparams.title_size + hparams.body_size + 2),
            dtype="int32",
        )
        #(?,50,82)-->(?,50,400)
        click_news_presents = tf.keras.layers.TimeDistributed(titleencoder)(
    ...
    def _build_newsencoder(self, embedding_layer):
        ...
        """
        hparams = self.hparams
        input_title_body_verts = tf.keras.Input(
            shape=(hparams.title_size + hparams.body_size + 2,), dtype="int32"
        )

        sequences_input_title = tf.keras.layers.Lambda(lambda x: x[:, : hparams.title_size])(
            input_title_body_verts
        )
        sequences_input_body = tf.keras.layers.Lambda(
            lambda x: x[:, hparams.title_size: hparams.title_size + hparams.body_size]
        )(input_title_body_verts)
        input_vert = tf.keras.layers.Lambda(
            lambda x: x[
                      :,
                      hparams.title_size
        ...
        input_subvert = tf.keras.layers.Lambda(
        ...
        concate_repr = tf.keras.layers.Concatenate(axis=-2)(
        ...
        model = tf.keras.Model(input_title_body_verts, pred_title, name="news_encoder")
        return model
    ...
    def _build_nrms(self):
        ...
        his_input_body = tf.keras.Input(
            shape=(hparams.his_size, hparams.body_size), dtype="int32"
        )
        his_input_vert = tf.keras.Input(shape=(hparams.his_size, 1), dtype="int32")
        his_input_subvert = tf.keras.Input(shape=(hparams.his_size, 1), dtype="int32")
        pred_input_title = tf.keras.Input(
            shape=(hparams.npratio + 1, hparams.title_size), dtype="int32"
        )
        pred_input_body = tf.keras.Input(
            shape=(hparams.npratio + 1, hparams.body_size), dtype="int32"
        )
        pred_input_vert = tf.keras.Input(shape=(hparams.npratio + 1, 1), dtype="int32")
        pred_input_subvert = tf.keras.Input(shape=(hparams.npratio + 1, 1), dtype="int32")
        pred_input_title_one = tf.keras.Input(
            shape=(hparams.title_size,), dtype="int32"
        )
        pred_input_body_one = tf.keras.Input(shape=(hparams.body_size,), dtype="int32")
        pred_input_vert_one = tf.keras.Input(shape=(1), dtype="int32")
        pred_input_subvert_one = tf.keras.Input(shape=(1), dtype="int32")

        his_title_body_verts = tf.keras.layers.Concatenate(axis=-1)(
            [his_input_title, his_input_body, his_input_vert, his_input_subvert]
        )

        pred_title_body_verts = tf.keras.layers.Concatenate(axis=-1)(
            [pred_input_title, pred_input_body, pred_input_vert, pred_input_subvert]
        )

        pred_input_title_one_reshape = tf.keras.layers.Reshape((1, hparams.title_size))(pred_input_title_one)
        pred_input_body_one_reshape = tf.keras.layers.Reshape((1, hparams.body_size))(pred_input_body_one)
        pred_input_vert_one_reshape = tf.keras.layers.Reshape((1, 1))(pred_input_vert_one)
        pred_input_subvert_one_reshape = tf.keras.layers.Reshape((1, 1))(pred_input_subvert_one)

        pred_title_body_verts_one = tf.keras.layers.Concatenate(axis=-1)(
            [
        ...
        pred_title_one_reshape = tf.keras.layers.Reshape((-1,))(pred_title_body_verts_one)
        ...
        news_present = tf.keras.layers.TimeDistributed(self.newsencoder)(pred_title_body_verts)
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        hparams = self.hparams
        sequences_input_title = tf.keras.Input(shape=(hparams.title_size,), dtype="int32")
        ...
        y = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
            [embedded_sequences_title] * 3
        )
        pred_title = AttLayer2(hparams.attention_hidden_dim, seed=self.seed)(y)
        pred_title = tf.keras.layers.Reshape((1, hparams.filter_num))(pred_title)

        model = tf.keras.Model(sequences_input_title, pred_title, name="title_encoder")
        return model
    ...
    def _build_bodyencoder(self, embedding_layer):
        ...
        """
        hparams = self.hparams
        sequences_input_body = tf.keras.Input(shape=(hparams.body_size,), dtype="int32")
        ...
        # )(y)
        y = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
            [embedded_sequences_body] * 3
        )
        # y = layers.Dropout(hparams.dropout)(y)
        pred_body = AttLayer2(hparams.attention_hidden_dim, seed=self.seed)(y)
        pred_body = tf.keras.layers.Reshape((1, hparams.filter_num))(pred_body)

        model = tf.keras.Model(sequences_input_body, pred_body, name="body_encoder")
        return model
    ...
    def _build_vertencoder(self):
        ...
        """
        hparams = self.hparams
        input_vert = tf.keras.Input(shape=(1,), dtype="int32")

        vert_embedding = tf.keras.layers.Embedding(
            hparams.vert_num, hparams.vert_emb_dim, trainable=True
        )

        vert_emb = vert_embedding(input_vert)
        pred_vert = tf.keras.layers.Dense(
            hparams.filter_num,
            activation=hparams.dense_activation,
            bias_initializer=tf.keras.initializers.Zeros(),
            kernel_initializer=tf.keras.initializers.glorot_uniform(seed=self.seed),
        )(vert_emb)
        pred_vert = tf.keras.layers.Reshape((1, hparams.filter_num))(pred_vert)

        model = tf.keras.keras.Model(input_vert, pred_vert, name="vert_encoder")
        return model
    ...
    def _build_subvertencoder(self):
        ...
        """
        hparams = self.hparams
        input_subvert = tf.keras.Input(shape=(1,), dtype="int32")

        subvert_embedding = tf.keras.layers.Embedding(
            hparams.subvert_num, hparams.subvert_emb_dim, trainable=True
        )

        subvert_emb = subvert_embedding(input_subvert)
        pred_subvert = tf.keras.layers.Dense(
            hparams.filter_num,
            activation=hparams.dense_activation,
            bias_initializer=tf.keras.initializers.Zeros(),
            kernel_initializer=tf.keras.initializers.glorot_uniform(seed=self.seed),
        )(subvert_emb)
        pred_subvert = tf.keras.layers.Reshape((1, hparams.filter_num))(pred_subvert)
