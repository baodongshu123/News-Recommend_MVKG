...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder):
        ...
        click_news_presents_drop = layers.Dropout(hparams.dropout)(click_news_presents)
        #(?,?,400)
        y = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
            [click_news_presents_drop] * 3
        )
        click_news_presents_drop_self = layers.Dropout(hparams.dropout)(y)
        #(?,400)
        user_present = AttLayer2(hparams.attention_hidden_dim, seed=self.seed)(
            click_news_presents_drop_self
        )
    ...
    def _build_newsencoder(self, embedding_layer):
        ...
        # y_drop_vert_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)([drop_vert_repr, drop_vert_repr, drop_vert_repr])
        # y_drop_subvert_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)([drop_subvert_repr, drop_subvert_repr, drop_subvert_repr])
        ...
        concate_repr = layers.Concatenate(axis=-2)(
            [drop_title_repr, drop_body_repr, drop_vert_repr, drop_subvert_repr]
        )
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        y1 = layers.Dropout(hparams.dropout)(embedded_sequences_title)
        # y = layers.Conv1D(
        #     hparams.filter_num,
        #     hparams.window_size,
    ...
    def _build_bodyencoder(self, embedding_layer):
        ...
        y2 = layers.Dropout(hparams.dropout)(embedded_sequences_body)
        # y = layers.Conv1D(
        #     hparams.filter_num,
        #     hparams.window_size,
        #     activation=hparams.cnn_activation,
