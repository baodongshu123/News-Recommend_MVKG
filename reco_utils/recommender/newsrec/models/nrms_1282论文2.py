...
class NRMSModel(BaseModel):
    ...
    def _build_newsencoder(self, embedding_layer,entity_embedding_layer):
        ...
        """
        hparams = self.hparams
        input_title_body_verts = keras.Input( shape=(hparams.title_size + hparams.body_size + 2+ hparams.entity_size,), dtype="int32")
        ...
        input_subvert = layers.Lambda(lambda x: x[:, hparams.title_size + hparams.body_size + 1:hparams.title_size+ hparams.body_size+ 1 + 1])(input_title_body_verts)
        sequence_input_entity = layers.Lambda(lambda x: x[:, hparams.title_size + hparams.body_size + 1 + 1:])(input_title_body_verts)
