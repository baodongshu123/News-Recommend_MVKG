...
# 想在MVKG 用户建模上改一下，在注意机制前加上GRU网络，结果效果不理想 0.6，0.64，0.65，0.66，没有到不到MVKG的效果
# 想在MVKG 用户建模上改一下，在注意机制前加上RNN网络，结果效果不理想
import numpy as np
...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder):
        ...
        #     click_news_presents
        # )
        # user_present = tf.reduce_mean(click_news_presents, axis=1)
        print(user_present)   #shape=(?, 400)
    ...
    def _build_newsencoder(self, embedding_layer,entity_embedding_layer):
        ...
        concate_repr = layers.Concatenate(axis=-2)(
            ...
            # [title_repr, body_repr, vert_repr,subvert_repr] 缺知识
            ...
            # [title_repr, body_repr, subvert_repr, entity_repr]缺主类别
             [title_repr, body_repr, vert_repr,entity_repr]  #缺子类别
        )
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        # pred_title = tf.keras.backend.mean(y, axis=1)
        # pred_title = (lambda x:tf.reduce_mean(x,axis=1))(y)
        # pred_title=tf.reshape(y,(-1,400))
        print("pred_title")
        print(pred_title)
        pred_title = layers.Reshape((1, hparams.filter_num))(pred_title)
        # pred_title = tf.reshape(pred_title, (-1, 1, 400))
        print("pre2")
        print(pred_title)
