...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder):
        ...
        his_input_title_body_verts = keras.Input(
            shape=(hparams.his_size, hparams.title_size + hparams.body_size +1+hparams.entity_size + hparams.neighbor_size
                   + hparams.entity_size + hparams.neighbor_size),
            dtype="int32",
        )
    ...
    def _build_newsencoder(self, embedding_layer,entity_embedding_layer):
        ...
        ###自己写的代码LSMV（实体与邻居实体交互完成之后再与内容交互）
        #co-Att模块
        # coatt = CoAtt(attention_hidden_dim=hparams.attention_hidden_dim, seed=self.seed)([title_cnn, body_mh])  # (?,80,400)
        # # #新闻内容与其标题实体及标题邻居实体的交互
        # entity_neighbor = AGATLayer(entity_size=hparams.entity_size,neighbor_size=hparams.neighbor_size,)([sequence_input_title_entity,sequence_input_title_neighbor])  # (?,40,200)
        # title_entity_con = Te_co_Att(entity_embedding_layer=entity_embedding_layer)([coatt, entity_neighbor])
        # title_en_ne_att = AttLayer2(hparams.filter_num, seed=self.seed)(title_entity_con)  # (?,200)
        # title_en_ne_repr = Reshape_tensor(hparams.filter_num)(title_en_ne_att)
        # # #新闻内容与其摘要实体及摘要邻居实体的交互
        # entity_neighbor = AGATLayer(entity_size=hparams.entity_size,neighbor_size=hparams.neighbor_size)([sequence_input_abstract_entity,sequence_input_abstract_neighbor])
        # abstract_entity_neighbor_con = Te_co_Att(entity_embedding_layer=entity_embedding_layer)([coatt,entity_neighbor,])
        # abstract_en_ne_att = AttLayer2(hparams.filter_num, seed=self.seed)(abstract_entity_neighbor_con)
        # abstract_en_ne_repr = Reshape_tensor(hparams.filter_num)(abstract_en_ne_att)
