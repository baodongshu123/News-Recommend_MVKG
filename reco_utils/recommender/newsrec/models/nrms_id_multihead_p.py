...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder):
        ...
        his_input = his_input_title_body_verts
        user_indexes = keras.Input(shape=(1,), dtype="int32")
        nuser_id = layers.Reshape((1, 1))(user_indexes)
        repeat_uids = layers.Concatenate(axis=-2)([nuser_id] * hparams.his_size)
        his_title_uid = layers.Concatenate(axis=-1)([his_input_title_body_verts, repeat_uids])
        ...
        user_present = PersonalizedAttentivePooling(
            ...
            seed=self.seed,
        )([click_news_presents, layers.Dense(hparams.attention_hidden_dim)(u_emb)])
    ...
    def _build_newsencoder(self, embedding_layer,user_embedding_layer):
        ...
        input_title_body_verts = keras.Input(
            shape=(hparams.title_size + hparams.body_size + 3,), dtype="int32"
        )

        sequences_input_title = layers.Lambda(lambda x: x[:, : hparams.title_size],output_shape=(30,))(
            input_title_body_verts
        )
        sequences_input_body = layers.Lambda(
            lambda x: x[:, hparams.title_size: hparams.title_size + hparams.body_size],output_shape=(50,)
        )(input_title_body_verts)
        ...
        input_subvert = layers.Lambda(
            lambda x: x[:, hparams.title_size + hparams.body_size + 1:-1],output_shape=(1,)
        )(input_title_body_verts)
        input_user_index = layers.Lambda(
            lambda x: x[:, hparams.title_size + hparams.body_size + 2:],output_shape=(1,)
        )(input_title_body_verts)
        ...
        #     [title_repr, title_repr, title_repr])
        # y_drop_title_repr_shape = layers.Reshape((1, hparams.filter_num))(y_drop_title_repr)
        #
        # y_drop_body_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [body_repr, body_repr, body_repr])
        # y_drop_body_repr_shape = layers.Reshape((1, hparams.filter_num))(y_drop_body_repr)

        #drop_out层
        # y_drop_title_repr = layers.Dropout(hparams.dropout)(y_drop_title_repr_shape)
        # y_drop_body_repr = layers.Dropout(hparams.dropout)(y_drop_body_repr_shape)
        '''
            y_drop_title_repr:(?,1,400)
            y_drop_body_repr:(?,1,400)
            vert_repr:(?,1,400)
            subvert_repr:(?,1,400)
        '''
        ...
        pred_title = PersonalizedAttentivePooling(
            ...
            hparams.attention_hidden_dim,
            seed=self.seed,
        )([concate_repr, layers.Dense(hparams.attention_hidden_dim)(u_emb)])
        # concate_repr = layers.Concatenate(axis=-2)(
        #     [y_drop2_title_repr, y_drop2_body_repr, drop_vert_repr, drop_subvert_repr]
        # )
        # y = layers.Dropout(hparams.dropout)(concate_repr)
        # y_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [y, y, y])
        # y_pre = layers.Dropout(hparams.dropout)(y_repr)
        # pred_title = AttLayer2(hparams.attention_hidden_dim, seed=self.seed)(y_pre)
        # drop_title_repr = layers.Dropout(hparams.dropout)(title_repr)
        # drop_body_repr = layers.Dropout(hparams.dropout)(body_repr)
        # drop_vert_repr = layers.Dropout(hparams.dropout)(vert_repr)
        # drop_subvert_repr = layers.Dropout(hparams.dropout)(subvert_repr)

        # y_drop_title_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [drop_title_repr, drop_title_repr, drop_title_repr])
        # y_drop_body_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [drop_body_repr, drop_body_repr, drop_body_repr])
        # y_drop_vert_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [drop_vert_repr, drop_vert_repr, drop_vert_repr])
        # y_drop_subvert_repr = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [drop_subvert_repr, drop_subvert_repr, drop_subvert_repr])
        ...
        # y_drop2_vert_repr = layers.Dropout(hparams.dropout)(y_drop_vert_repr)
        # y_drop2_subvert_repr = layers.Dropout(hparams.dropout)(y_drop_subvert_repr)

        # concate_repr = layers.Concatenate(axis=-2)(
        #     [y_drop2_title_repr, y_drop2_body_repr, y_drop2_vert_repr, y_drop2_subvert_repr]
        # )
    ...
    def _build_nrms(self):
        ...
        his_title_body_verts = layers.Concatenate(axis=-1)(
            [his_input_title, his_input_body, his_input_vert, his_input_subvert,his_repeat_uindex]
        )



        pred_title_body_verts = layers.Concatenate(axis=-1)(
            [pred_input_title, pred_input_body, pred_input_vert, pred_input_subvert,repeat_uindex]
        )
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        model = keras.Model(sequences_input_title, y_drop_title_repr_shape, name="title_encoder")
        return model
    ...
    def _build_bodyencoder(self, embedding_layer):
        ...
        model = keras.Model(sequences_input_body, y_drop_body_repr_shape, name="body_encoder")
        return model
