...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder,user_embedding_layer):
        ...
        his_input_title_body_verts = keras.Input(
            shape=(hparams.his_size,hparams.title_size + hparams.body_size + 3,), dtype="int32"
        )
    ...
    def _build_newsencoder(self, embedding_layer,user_embedding_layer):
        ...
        u_emb = layers.Reshape((1, hparams.user_emb_dim,))(
        ...
        concate_repr = layers.Concatenate(axis=-2)(
            [y_drop2_title_repr, y_drop2_body_repr, drop_vert_repr, drop_subvert_repr,u_emb]
        )
        #添加user_id
        ...
        # )
        # y = layers.Concatenate(axis=-2)([concate_repr, u_emb])
        # y = layers.Dropout(hparams.dropout)(y)
        # y = SelfAttention(hparams.head_num, hparams.head_dim, seed=self.seed)(
        #     [y] * 3
        # )
    ...
    def _build_nrms(self):
        ...
        repeat_uids = layers.Concatenate(axis=-2)([nuser_index] * hparams.his_size)

        his_title_body_verts = layers.Concatenate(axis=-1)(
            [his_input_title, his_input_body, his_input_vert, his_input_subvert,repeat_uids]
        )
        pred_repeat_uids = layers.Concatenate(axis=-2)([nuser_index] * (hparams.npratio + 1))
        pred_title_body_verts = layers.Concatenate(axis=-1)(
            [pred_input_title, pred_input_body, pred_input_vert, pred_input_subvert,pred_repeat_uids]
        )
