...
class NRMSModel(BaseModel):
    ...
    def _build_userencoder(self, titleencoder):
        ...
        his_input_title_body_verts = keras.Input(
            shape=(hparams.his_size, hparams.title_size + hparams.body_size + 1 + hparams.entity_size),
            dtype="int32",
        )
        #(?,50,82)-->(?,50,400)
        ...
        #     y
        # )
        # user_present_reshap = layers.Reshape((1,400))(user_present)
        # short_uemb_reshap = layers.Reshape((1,400))(short_uemb)
        # user_present_concat = layers.Concatenate(axis=-2)([user_present_reshap,short_uemb_reshap])
    ...
    def _build_newsencoder(self, embedding_layer, entity_embedding_layer):
        ...
        input_title_body_verts = keras.Input(
            shape=(hparams.title_size + hparams.body_size + 1+ hparams.entity_size,), dtype="int32"
        )
        ...
        concate_repr = layers.Concatenate(axis=-2)(
            [title_repr, body_repr, vert_repr,entity_repr]
        )
    ...
    def _build_nrms(self):
        ...
        pred_input_entity_one = keras.Input(shape=(hparams.entity_size,), dtype="int32")

        his_title_body_verts = layers.Concatenate(axis=-1)(
            [his_input_title, his_input_body, his_input_vert,his_input_entity]
        )

        pred_title_body_verts = layers.Concatenate(axis=-1)(
            [pred_input_title, pred_input_body, pred_input_vert,pred_input_entity]
        )
        ...
        self.newsencoder = self._build_newsencoder(embedding_layer, entity_embedding_layer)
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        y = layers.Dropout(hparams.dropout)(y) #(?,30,400)
