...
class create_hparms:
    def __init__(self,flags):
        self.data_format = flags.get("data_format", None)
        self.iterator_type = flags.get("iterator_type", None)
        self.support_quick_scoring = flags.get("support_quick_scoring", False)
        self.wordEmb_file = flags.get("wordEmb_file", None)
        self.wordDict_file = flags.get("wordDict_file", None)
        self.userDict_file = flags.get("userDict_file", None)
        self.vertDict_file = flags.get("vertDict_file", None)
        self.subvertDict_file = flags.get("subvertDict_file", None)
        # models
        self.title_size = flags.get("title_size", None)
        self.body_size = flags.get("body_size", None)
        self.word_emb_dim = flags.get("word_emb_dim", None)
        self.word_size = flags.get("word_size", None)
        self.user_num = flags.get("user_num", None)
        self.vert_num = flags.get("vert_num", None)
        self.subvert_num = flags.get("subvert_num", None)
        self.his_size = flags.get("his_size", None)
        self.npratio = flags.get("npratio")
        self.dropout = flags.get("dropout", 0.0)
        self.attention_hidden_dim = flags.get("attention_hidden_dim", 200)
        # nrms
        self.head_num = flags.get("head_num", 4)
        self.head_dim = flags.get("head_dim", 100)
        # naml
        self.cnn_activation = flags.get("cnn_activation", None)
        self.dense_activation = flags.get("dense_activation", None)
        self.filter_num = flags.get("filter_num", 200)
        self.window_size = flags.get("window_size", 3)
        self.vert_emb_dim = flags.get("vert_emb_dim", 100)
        self.subvert_emb_dim = flags.get("subvert_emb_dim", 100)
        # lstur
        self.gru_unit = flags.get("gru_unit", 400)
        self.type = flags.get("type", "ini")
        # npa
        self.user_emb_dim = flags.get("user_emb_dim", 50)
        # train
        self.learning_rate = flags.get("learning_rate", 0.001)
        self.loss = flags.get("loss", None)
        self.optimizer = flags.get("optimizer", "adam")
        self.epochs = flags.get("epochs", 10)
        self.batch_size = flags.get("batch_size", 1)
        # show info
        self.show_step = flags.get("show_step", 1)
        self.metrics = flags.get("metrics", None)


def create_hparams2(flags):
    ...
    """
    print("*************",flags.get("wordEmb_file", None))
    return tf.compat.v1.training.HParams(
        # data
